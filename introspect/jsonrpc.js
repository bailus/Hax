...
var JSONRPC = (function (window, undefined) {
	...
	connect = function (url, callback) {
		...
		}, JSONRPC[transport](address, callback));
	}
	...
	JSONRPC.ajax = function (url, callback) {
	...
	JSONRPC.websocket = function (url, callback) {
		  ...
		  connectSocket = function () {
			...
			socket.onmessage = function (message) {
				...
				if (data.id) { //json-rpc message
					...
					if (onAllNotifications instanceof Function) onAllNotifications(data);
				}
				sendNext();
			};
